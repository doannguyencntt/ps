  ...
  methods: {
    ...
    ok() {
      ...
      if (this.password.token) {
        apiUserClient.confirmTokenOrgInvitation({token: this.password.token}).then((response) => {
          if (response) {
            this.isLoading = true
            ...
            if (response.status === 'Decline') {
              this.notify = {
                type: 'danger',
                content: 'You have denied this invitation.'
              }
            } else if (response.status === 'Accept') {
              this.notify = {
                type: 'success',
                content: 'You have accepted this invitation already.'
              }
            } else {
              if (!this.needChangePassword) {
                ...
                this.notify = {
                  type: 'success',
                  content: 'Your account has been activated for the new organization.'
                }
              } else {
                // user is not exsisted
                this.notify = {
                  type: 'success',
                  content: 'Your account has been activated for the new organization. Also please create your first password. You can skip this step and use password recovery feature.'
                }
              }
            }
          } else {
