...
import { inviteUserToOrgApi } from '@/services/userOrg'
...
export default {
  ...
  mixins: [vToast, commons, checkingUserPermissions('org', true, { name: 'ps-OrgUsers', params: { type: 'org' } })],
  computed: {
    ...mapGetters({
    ...
    async inviteUser () {
      ...
      if (orgId) {
        let promiseList = this.invitedMembers.map((user, index) => {
          ...
          user.activation_link_template = window.location.protocol + '//' + window.location.host + '/#/ps/organizations/invitation?token={token}'
          return new Promise((resolve, reject) => {
            inviteUserToOrgApi(orgId, user).then(response => {
