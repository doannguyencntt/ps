...
const ROOT_API = process.env.VUE_APP_DTD_API_BASE_URL

const dtdAxios = axios.create({
  baseURL: ROOT_API,
  timeout: 600000
})

dtdAxios.interceptors.request.use(function (config) {
...
dtdAxios.interceptors.response.use(function (response) {
  return response
}, function (error) {
  // Server error
