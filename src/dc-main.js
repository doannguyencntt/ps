...
import dcModule from 'plat-dc-lib'
import 'plat-dc-lib/dist/dc-admin.css'
...
import DC_API from '@/services/dcAxios'
...
const DC_API_CONFIG = {
  baseUrl: process.env.VUE_APP_DC_API_BASE_URL || ''
}
const DC_URL_CONFIG = {
  ASIN_SAMPLE_FILE: process.env.VUE_APP_ASIN_SAMPLE_FILE || '',
  PO_SAMPLE_FILE: process.env.VUE_APP_PO_SAMPLE_FILE || '',
  MFN_PRIME_ORDERS_SAMPLE_FILE: process.env.VUE_APP_MFN_PRIME_ORDERS_SAMPLE_FILE || '',
  STR_SAMPLE_FILE: process.env.VUE_APP_STR_SAMPLE_FILE || ''
}
const SDK_CONFIG = {
...
Vue.use(importSDK, { store, router, psNav })
importSDK.setAxios(DC_API)
...
// dc
Vue.use(dcModule, { store, router, layout, nav, retryOn401, DC_API_CONFIG, DS_API_CONFIG, customSidebar, DC_URL_CONFIG })
