    ...
    <b-row>
      <b-col lg="12" sm="12">
        <b-card no-body>
              ...
              <tbody>
                <tr v-for="(item, index) in invitedMembers" :key="index">
                  <td>
                    ...
                    <div v-else>
                      <b-form-input ref="emailInput" type="email" name="email" placeholder="Email" v-model.trim="item.email" :class="{'is-invalid': $v.invitedMembers.$each[index].email.$dirty && $v.invitedMembers.$each[index].email.$anyError}"></b-form-input>
                    </div>
                  ...
                  <td>
                    <b-button @click.prevent="userToBeRemoved=index" v-b-modal.remove variant="danger" size="sm">
          ...
          <div slot="footer">
            <!-- getting from mixins -->
            <b-button :disabled="isGettingUser" v-on:click="inviteUser" variant="primary" size="sm" class="mr-1">
              <i class="icons icon-cursor"/> Invite
            </b-button>
          </div>
        </b-card>
      </b-col>
...
export default {
  ...
  mixins: [vToast, commons, checkingUserPermissions('client', true, { name: 'ps-UserList' })],
  computed: {
    ...mapGetters({
    ...
    async inviteUser () {
      ...
      if (clientID) {
        let promiseList = this.invitedMembers.map((user, index) => {
          ...
          user.activation_link_template = window.location.protocol + '//' + window.location.host + '/#/ps/clients/invitation?token={token}'
          return new Promise((resolve, reject) => {
            apiUser.inviteUser(clientID, user).then(response => {
              this.invitedSuccessList.push(user.email)
              this.vToasted('success', 'The invitation has been sent to ' + user.email)
              resolve(response)
            }).catch(error => {
