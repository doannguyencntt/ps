  ...
  <b-card>
    ...
    <div>
      <b-table
        ...
        :items="customRoles.results || []"
        :fields="userRole === 'STAFF' ? fields.filter(f => f.key !== 'action') : fields">
        <template v-slot:empty>
          <div class="text-center py-2" v-if="roleLoading">
            <i class="fa fa-circle-o-notch fa-spin"></i>
            Loading...
          </div>
          <div class="align-middle d-flex justify-content-center align-items-center" v-else>There are no custom roles to show</div>
        </template>
        ...
        <template v-slot:cell(action)="data">
          ...
          >
            ...
            <b-dropdown-item-button class="btn-sm p-0"
              :disabled="!canEnableManage(data.item)"
              @click="goToPage('EditRole', data.item.id)">
      ...
      <nav class="mt-3 custom-pagination" v-if="customRoles.count && customRoles.count > dataFilter.limit">
        <b-pagination
          align="center"
          @input="loadPage()"
          :total-rows="customRoles.count"
    ...
    <!-- <b-alert show v-else-if="isInitialized && (!customRoles.results || !customRoles.results.length)">No custom roles</b-alert> -->
    <b-modal
        id="remove"
...
<script>
import { getCustomRolesFn, deleteCustomRoleFn } from '@/services/rolesSvc'
...
export default {
  ...
  async mounted () {
    ...
    if (this.config.id) this.getRoles(this.config.id, this.dataFilter)
  ...
  methods: {
    ...
    getRoles (sizeId, dataFilter = {}) {
      const params = this.cleanObj({...dataFilter})
      this.roleLoading = true
      getCustomRolesFn(sizeId, params, this.config.type).then(res => {
    ...
    searchChange () {
      ...
      this.getRoles(this.config.id, this.dataFilter)
      const query = {
    ...
    removeCustomRole () {
      if (this.actionData && this.actionData.id) {
        deleteCustomRoleFn(this.config.id, this.actionData.id, this.config.type).then(res => {
          this.getRoles(this.config.id, this.dataFilter)
        }).catch(() => {
          this.vToasted('error', 'Can not remove this custom role. Please try again or contact administrator.')
        })
      }
    },
    goToPage (routeName = 'CreateRole', roleId = '') {
      const query = {}
      if (this.dataFilter.page) query.page = this.dataFilter.page
      if (this.dataFilter.option_filter) query.type = this.dataFilter.option_filter
  ...
  watch: {
    'config.id' (newVal, oldVal) {
      if (newVal && newVal !== oldVal) {
        this.getRoles(newVal, this.dataFilter)
